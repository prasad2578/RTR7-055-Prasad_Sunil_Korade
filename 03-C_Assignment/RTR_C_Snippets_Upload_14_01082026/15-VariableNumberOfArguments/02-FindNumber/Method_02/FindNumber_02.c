...
int main(void)
{
    ...
    printf("\n\n");
    FindNumber(PK_NUM_TO_BE_FOUND, PK_NUM_ELEMENTS, 3, 3, 9, 2, 3, 6, 9, 3, 1, 3);
    return(0);
}
...
void FindNumber(int PK_num_to_be_found, int PK_num, ... )
{
    ...
    PK_count = va_FindNumber(PK_num_to_be_found, PK_num, PK_numbers_list);
    if(PK_count == 0)
...
int va_FindNumber(int PK_num_to_be_found, int PK_num, va_list PK_list)
{
    int PK_count_of_num = 0;
    int PK_n;

    while(PK_num)
    {
        PK_n = va_arg(PK_list, int);
        if(PK_n == PK_num_to_be_found)
            PK_count_of_num++;
        PK_num--;
    }
    return(PK_count_of_num);
}
